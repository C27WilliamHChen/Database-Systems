...
# We expect your Mongo connection string in an environment variable called MONGO_URI.
# Example (in your terminal before running app.py):
...
if not MONGO_URI:
    raise RuntimeError("MONGO_URI environment variable is not set. "
                       "Run: export MONGO_URI='your_connection_string'")
...
def delete():
    """
    Bulk delete route.
    Receives a list of MongoDB _id strings from the checkboxes (name="delete_ids").
    """
    id_strings = request.form.getlist("delete_ids")
...
if __name__ == "__main__":
    # host=0.0.0.0 makes it reachable from Windows via localhost when using WSL
